...
Under this case of ``Coefficients Asymptotically Constant," the form assumed for the sum (1) is:
	\begin{equation} 
...
We then plot the distribution of zeros for exponential sum with collinear complex exponents, as shown in Figure 2.
...
Inspired by , we will illustrate this most general case by using the following exponential sum as an example:
\begin{align*}
&3e^{(8+2i)z} + (-9 + 12i)e^{(4+7i)z} + (2 + 1i) * e^{(-7+4i)z} + (-5) e^{(-6-6i)z} \\
&+ (6 - 7i)e^{(1-8i)z} + (8 - 5i) e^{(6-4i)z} + (3 - 9i) e^{(4+4i)*z} + 2 e^{(-2-4i)z}.
\end{align*}
We then plot the distribution of zeros of this exponential sum in Figure 2.
