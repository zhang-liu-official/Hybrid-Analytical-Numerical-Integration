%---------DO NOT EDIT THIS INDENTED SECTION
	...
	\author{Zhang Liu}
	\date{August 1, 2019}   % Using \today automatically updates to the document's build date
	\usepackage{mdframed}
	\usepackage{graphicx}
	\usepackage{wrapfig}
...
\begin{abstract}
	The aim of this document was to explain Langer's paper on the zeros of exponential sums \cite{Lan1931a}. We followed the general structure of the original paper and explained the main theorems for the cases that are relevant to our project. These cases were illustrated with corresponding plots for arbitrary examples. 
\end{abstract}
...
\end{thm}
	...
	\begin{figure}[htp]
	\begin{center}
		\includegraphics[width=10cm]{plot-general-real.png}
	\end{center}
...
\begin{figure}[htp]
	\begin{center}
		\includegraphics[width=10cm]{plot-collinear-complex-exponents.png}
	\end{center}
...
\begin{figure}[htp]
	\begin{center}
		\includegraphics[width=10cm]{plot-general-complex.png}
	\end{center}
	\caption{General complex constants.}
	\label{fig:figure4}
\end{figure}

A geometric method is used in the paper. Let it be supposed that the values $\bar c_j$ (conjugate to the exponents) have been plotted in the complex plane (Figure 5).

\begin{figure}[htp]
	\begin{center}
		\includegraphics[width=10cm]{non-convex-polygon.png}
	\end{center}
	\caption{Polygon joined by all the points.}
	\label{fig:figure5}
\end{figure}

Construct the polygon $P$ which (i), is convex, (ii), has vertices only at points of the set, and (iii), includes all points of the set either in its interior or on its perimeter. Given these three conditions, we obtain the polygon as shown in Figure 6. 


\begin{figure}[htp]
	\begin{center}
		\includegraphics[width=10cm]{convex-polygon.png}
	\end{center}
...
This way, we transform this most general case to the previous case of collinear complex exponents. It then follows from Theorem 5 that the zeros of $\Phi(z)$ within the sector are asymptotically confined to one or more strips which are parallel or approach parallelism with the normal to the associated side of the polygon, as more formally stated in Theorem 6 below.
...
\begin{figure}[htp]
	\begin{center}
		\includegraphics[width=10cm]{overlaid-plot.png}
	\end{center}
